...
- **Scope enforcement** — tokens can be restricted to specific repositories and permission levels; GitHub's native token permissions provide a secondary defence for API endpoints not yet mapped in ghp's rules table (see [Token Scoping](features/token-scoping.md))
